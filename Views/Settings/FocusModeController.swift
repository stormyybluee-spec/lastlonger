...
struct LastLongerShortcuts: AppShortcutsProvider {
    static var appShortcuts: [AppShortcut] {
        ...
        AppShortcut(
            intent: LogHoldIntent(),
            phrases: ["Log a hold in \(.applicationName)", "\(.applicationName) log hold"],
            shortTitle: "Log a hold",
            systemImageName: "flame.fill"
        )
        AppShortcut(
            intent: LogRecoverIntent(),
            phrases: ["Log a recover in \(.applicationName)", "\(.applicationName) log recover"],
            shortTitle: "Log a recover",
            systemImageName: "wind"
        )
        AppShortcut(
            intent: EmergencyIntent(),
            phrases: ["Emergency in \(.applicationName)", "\(.applicationName) emergency"],
            shortTitle: "Emergency protocol",
            systemImageName: "exclamationmark.triangle.fill"
        )
        AppShortcut(
            intent: EndSessionIntent(),
            phrases: ["End session in \(.applicationName)", "\(.applicationName) end session"],
